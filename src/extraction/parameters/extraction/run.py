# src/extraction/parameters/extraction/run.py
...
class ExtractionRunner:
    ...
    def screen_parameters(
        ...
        for parameter_class in self.parameter_classes:
            ...
            max_screening_iterations = 50

            while not success and iteration < max_screening_iterations:
                success = True
                iteration += 1
                ...
                try:
                    ...
                    raise RuntimeError(
                        f"LLM screening failed for article {article_id} "
                        f"and parameter {parameter_class}: {e}"
                    ) from e
                            ...
                            if function_args != {}:
                                try:
                                    ...
                                    result = extractor.return_result(
                                        parameter=parameter_class,
                                        errors=[
                                            f"{tool['function']['name']} failed with an error:\n{e}"],
                                    )
            ...
            if iteration >= max_screening_iterations:
                self.logger.warning(
                    f"Screening for {parameter_class} in article {article_id} "
                    f"hit max iteration limit ({max_screening_iterations})"
                )
    ...
    def extract_structured_parameters_with_tool(
        ...
        while not success and iteration < max_iterations:
            ...
            try:
                ...
                trace["error"] = str(e)
                raise RuntimeError(
                    f"LLM extraction failed for article {article_id} "
                    f"and parameter {parameter}: {e}"
                ) from e
        ...
        if iteration >= max_iterations:
            self.logger.warning(
                f"Extraction for {parameter} in article {article_id} "
                f"hit max iteration limit ({max_iterations})"
            )
    ...
    def write_final_parameters(self):
        raw_parameters_path = os.path.join(self.log_dir, "results", "raw_parameters.jsonl")

        if not os.path.exists(raw_parameters_path):
            self.logger.warning(
                f"No parameters were extracted. {raw_parameters_path} does not exist."
            )
            return

        with open(raw_parameters_path, "r") as f:
