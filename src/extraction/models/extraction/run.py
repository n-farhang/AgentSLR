# src/extraction/models/extraction/run.py
        ...
        while iteration < max_iterations:
            ...
            try:
                ...
                for tc in (message.tool_calls or []):
                    if tc.type != "function":
                        continue
                    if tc.function.name != "extract_model_data":
                        continue
                    tool_called = True
                    call_id = tc.id
                ...
                self._save_trace(article_id, trace)
                raise RuntimeError(
                    f"Model extraction failed for article {article_id}: {e}"
                ) from e
            ...
            try:
                ...
                for tc in (message.tool_calls or []):
                    if tc.type != "function":
                        continue
                    call_id = tc.id
                    if tc.function.name == "extract_model_provenance":
                        try:
                            function_args = json.loads(tc.function.arguments)
                ...
                raise RuntimeError(
                    f"Model provenance extraction failed for article {article_id}, "
                    f"model {model_index}: {e}"
                ) from e
